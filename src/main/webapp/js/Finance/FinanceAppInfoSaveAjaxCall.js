...
$("#legacyAppInfoSave").click(function(e)
{
    ...
    var enteredAppName =$('#financeappname').val();
    var selectedAppName = $('#nameInput').val();
    if (enteredAppName===selectedAppName){
    if(checkMandatory){
        ...
        notification("warning","Selected Application Name And Finance Application Name Dosn't Match.","Warning : ");
        e.preventDefault();
    }
...
function archiveLegacyAppInfoAjaxCallUpdate(JsonString,checkMandatory,e)
{
    ...
    $.ajax({
        ...
        type: 'POST',
        data : {JsonString : JsonString, checkMandatory : checkMandatory, ID:selectedId,Opportunity:selectedName },
        async: false,
        dataType: "json",
